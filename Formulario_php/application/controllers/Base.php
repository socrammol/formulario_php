...
class Base extends CI_Controller{
	...
	{
		//recupera os contatos atraves do model
		$conttatos = $this->contatos_model->GetAll('nome');
		//passa os contatos para o array qye sera enviado a home
		$dados['contatos'] = $this->contatos_model->Formatar($conttatos);
	...
	public function Salvar(){
		//Recuéra os contatos atraves do model
		$contatos = $this->contatos_model->GetAll('nome');
		//passa os contatos para o array que sera enviado a home
		$dados['contatos'] = $this->contatos_model->Formatar($contatos);
		//executa o processo de validação do formulario
		...
		if ($validacao){
			...
			if (!$status){
				$this->sesson->set_flashdata('error', 'Não foi possível inserir o contato.');
			...
			$this->session->setflashdata('error', validation_errors('<p>','</p>'));
		}
		//carrega a home
		$this->load->view('home',$dados);
	}
